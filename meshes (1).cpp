...
void Meshes::UCreateMemphisPyramid(GLMesh& mesh)
{
	// Define the vertices and indices for the Memphis Pyramid
	GLfloat vertices[] = {
		// Base
	...
	// Bind and initialize vertex buffer for vertices
	glBindBuffer(GL_ARRAY_BUFFER, mesh.vbos[0]);
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);

	// Set up attribute pointers for the vertex buffer
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), (GLvoid*)0);
	glEnableVertexAttribArray(0);

	// Bind and initialize vertex buffer for indices
